...
| `cmdCopyToClipboard` | `clipboardMsg{ok}` | copies via the OS's `Clipboard` implementation ([ui/clipboard.go](../ui/clipboard.go)): `clip.exe` on Windows; `wl-copy`/`xclip`/`xsel` (first found on `PATH`) on Linux, failing with a descriptive error if none are installed |
