...
func (tm *TunnelManager) Connect(server config.ServerConfig, signer ssh.Signer) error {
	...
	if server.HostKeyFingerprint == "" {
		return fmt.Errorf("gateway host key not verified for %q — trust its fingerprint before connecting", server.Name)
	}
...
// disconnectTimeout bounds how long Disconnect waits for at.close() before
// giving up on the caller's behalf. close() does network I/O over the
// existing SSH connection (a second session for the iptables cleanup
// command, then closing the tun channel/connection) with no deadline of its
// own; if the server or transport is wedged, that can block indefinitely,
// which previously left the UI stuck showing "disconnecting" forever.
// close() itself still runs to completion in the background - only the
// caller stops waiting on it.
const disconnectTimeout = 8 * time.Second

// sshCleanupTimeout bounds the server-side iptables cleanup session inside
// close() - see its call site for why this needs to force-close the
// connection rather than just move on.
const sshCleanupTimeout = 3 * time.Second
...
func (tm *TunnelManager) Disconnect() error {
	...
	case <-time.After(disconnectTimeout):
		logging.Errorf("tunnel_disconnect_timeout adapter=%s - teardown still running in background", stableAdapterName)
	}
	return nil
}
...
func (at *activeTunnel) close() {
	...
	// Clean up server-side iptables rules before closing SSH, but don't let
	// this block teardown indefinitely: if the cleanup session doesn't
	// finish quickly, force-close the connection instead of waiting on it.
	// A connection left dangling here keeps the server's tunnel slot marked
	// busy, blocking reconnects on the same slot until the dead connection
	// eventually times out on its own (which can take a very long time).
	if at.sshConn != nil {
		...
		select {
		case <-cleanupDone:
		case <-time.After(sshCleanupTimeout):
			logging.Errorf("tunnel_cleanup_timeout adapter=%s - forcing ssh connection closed", stableAdapterName)
		}
		// Closing the connection unblocks the goroutine above if it's still
		// stuck in NewSession/Run, and guarantees the server sees the
		// connection actually die even when the graceful path above hung.
		_ = at.sshConn.Close()
	}
