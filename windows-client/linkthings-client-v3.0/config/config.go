...
type ServerConfig struct {
	...
	SSHTunnel  int    `json:"sshTunnel"`     // Tunnel slot number (0-255, default 0 for multi-user support)
	...
	// HostKeyFingerprint pins the gateway's SSH host key (SHA256 fingerprint,
	// same format as ssh-keygen -lf), captured via a TOFU probe and confirmed
	// by the user on first connect. Empty means "not yet trusted" — every SSH
	// dial for this profile (tunnel connect, Test, the SSH-auth flow) refuses
	// to proceed until this is set, rather than falling back to an
	// unverified connection. See package hostkey.
...
func (s *ServerConfig) Validate() error {
	...
	if s.SSHTunnel < 0 || s.SSHTunnel > 255 {
		return fmt.Errorf("SSH tunnel slot must be 0-255, got %d", s.SSHTunnel)
	}
	if s.MTU == "" {
