...
- `linkthings-client-v3.0/` — **the actively developed client, start here for new work.** A fork of `linkthings-client-v2/` (module `linkthings.io/client-v3`) that replaces its Bubble Tea TUI with a Wails v3 + Vue 3 + Tailwind desktop GUI ("Nocturne" design system — see `linkthings-client-v3.0/design_handoff_linkthings_client/`), adds a system tray with background-run behavior, and replaces the HTTP OTP-provisioning flow with a direct SSH key-authorization flow. All of v2's `config`/`keymgmt`/`logging`/`paths`/`tunnel`/elevation code carried over unchanged. See `linkthings-client-v3.0/CLAUDE.md` for its own architecture docs.
- `linkthings-client-v2/` — **previous-generation TUI, kept buildable for reference/comparison while v3 stabilizes; not where new work happens.** A cross-platform (Windows + Linux) TUI SSH VPN client (module `linkthings.io/client-v2`), forked from `linkthings-client/` onto a `Platform`/`Elevator`/`Clipboard` interface seam so each OS implements a small set of primitives instead of duplicating the SSH/routing/framing orchestration per platform. See `linkthings-client-v2/CLAUDE.md` for its own architecture docs — don't assume the parent sections below (written for the legacy client) describe it.
- `linkthings-client/` — **frozen legacy v0.2, Windows-only. Do not modify — kept for reference only.** The original shipping product (module `linkthings.io/client`) that `linkthings-client-v2/` was forked from.
